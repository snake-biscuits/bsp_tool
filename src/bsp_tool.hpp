...
namespace id_software {
    const uint32_t FILE_MAGIC = MAGIC('I', 'B', 'S', 'P');
...
namespace valve {
    const int FILE_MAGIC = MAGIC('V', 'B', 'S', 'P');
    struct LumpHeader { uint32_t offset, length, version, uncompressed_size; };
...
namespace respawn {
    const uint32_t FILE_MAGIC = MAGIC('r', 'B', 'S', 'P');
    struct LumpHeader { uint32_t offset, length, version, uncompressed_size; };
...
struct GameLumpHeader { uint32_t id; uint16_t flags, version; uint32_t offset, length; };

// GAME_LUMP_SPRP = MAGIC('P', 'R', 'P', 'S')
