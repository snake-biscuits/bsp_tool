...
class MappedArray:
    ...
    def as_cpp(self, inline_as: str = None, one_liner_limit: int = 80) -> str:
        ...
        if len(self._classes) + len(self._bitfields) == 0 and isinstance(self._mapping, list):  # basic types only!
            if len(set(common.split_format(self._format))) == 1:
                one_liner = True  # struct MappedArray { float x, y, z; };
            if len(set(common.split_format(self._format))) * 8 + sum(map(len, self._attr_formats)) <= one_liner_limit - 30:
        ...
        for attr, attr_format in self._attr_formats.items():
            ...
            if not isinstance(value, (Iterable, bitfield.BitField)):  # basic type
                if not attr_format[-1] == "s":
                    attrs.append(f"{common.type_LUT[attr_format]} {attr};")
                else:
                    attrs.append(f"char {attr}[{attr_format[:-1]}];")
            else:  # sub-struct
                if isinstance(value, MappedArray):
                    attrs.extend(value.as_cpp(
                        inline_as=attr,
                        one_liner_limit=one_liner_limit - 4).split("\n"))
                elif isinstance(value, bitfield.BitField):  # gets skipped sometimes?
                    attrs.append(value.as_cpp(inline_as=attr))
                elif attr in self._classes:  # enum.IntFlags shouldn't end up here
                    kwargs = dict(
                        _mapping=self._mapping[attr],
                        _format="".join(attr_format))
                    limit = one_liner_limit - 4
                    attrs.extend(
                        MappedArray(*value, **kwargs).as_cpp(
                            inline_as=attr,
                            one_liner_limit=limit).split("\n"))
                # TODO: array of struct (List[MappedArray] mapping)
                    ...
                    if not attr_format[0][-1] == "s":
                        attrs.append(f"{common.type_LUT[attr_format[0]]} {attr}[{length}];")
                    else:
                        attrs.append(f"char {attr}[{attr_format[0][:-1]}][{length}];")
                else:
                    raise NotImplementedError(
                        f"Unexpected type for {attr}: {type(value)}")
        if one_liner:
            # compress inner
            ...
            if inline_as is None:
                return "".join([
                    f"struct {self.__class__.__name__}" + " { ", inner, " };"])
            else:
